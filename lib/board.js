...
module.exports = {
    list : function(request,response){
        db.query(`SELECT count(*) as total FROM board`,function(error, nums){
            finalindex = nums[0].total - 1
            var numPerPage = 2;
            var pageNum = request.params.pNum
            var offs = (pageNum-1)*numPerPage;
            totalPages = Math.ceil(nums[0].total / numPerPage);
        ...
        request.on('end', function() {
            ...
            db.query(`
                INSERT INTO board (loginid, name, date, content, title, password)
                    VALUES(?, ?, ?, ?, ?, ?)`,
                [post.id, post.nmh, dateOfEightDigit(),post.content, post.title, post.password], function(error, result) {
                    if(error) {
                        throw error;
                    }
    ...
    update_process : function(request, response) {
        ...
        request.on('end', function() {
            ...
            db.query('UPDATE board SET title=?, content=?, date=? WHERE id=?',
                [post.title, post.content, dateOfEightDigit(), post.id], function(error, result) {
                response.writeHead(302, {Location: `/board/view/${post.id}/${post.pNum}`});
                response.end();
            });
        });
    },
    delete : function(request, response) {
        ...
        db.query('DELETE FROM board WHERE id = ?', [bNum], function(error, result) {
            ...
            if(pNum==totalPages && finalindex%2==0){pNum -=1}
